...
public class BaseController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, 
		...
		try {
			...
			if( ret!=null ){
				...
				System.out.println("{SERVER} "+ret+"执行完成");
			}else{
	...
	protected void redirect( String url, HttpServletRequest req, 
		...
		//{2} 执行请求重定向。
		System.out.println("{SERVER} 执行重定向到"+ctxPath + url);
		System.out.println("{SERVER} 项目根映射地址"+req.getContextPath());
	...
	protected void forward( String uri, HttpServletRequest req, 
		...
		throws IOException, ServletException{
		//{1} 执行内部转发。
		System.out.println("{SERVER} 执行内部转发到"+uri);
		System.out.println("{SERVER} 项目根映射地址"+req.getContextPath());
